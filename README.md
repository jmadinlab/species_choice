...
We are also exploring *density* in PC space, which weighs species from clusters of species in trait space more (or less) heavily (scaled between 0 and 1). This needs discussion, but presumably species cluster evoluntionarily in certain regions of trait space for a reason, and so perhaps one scenarios should look to reflect this.
